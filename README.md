...
- 自动指标：`identity_vs_ref`（同身份倾向）+ `residual_vs_target`（旧身份残
  ...
  表情/姿势跟随度，<0.1 为精确）+ `hair_vs_ref/target`（脸框上方发区 HSV 直
  方图交集——发型跟随谁的硬指标）
- **发型-表情耦合律（实测）**：InstantID 系表情跟参考图的档发型也跟参考
  （hair_vs_ref 0.81-0.93）；表情跟被换脸图的档发型也跟被换脸图（0.74-0.98）
  ——`hair=True` 扩大重绘并不迁移发型（身份嵌入不含发型）。“表情跟目标 + 发型跟
  参考”需指令路线（`qwen_edit`，探针中）
