...
**`CHAIN_API_INFO.md`**（820 根目录）= 完整 API 文档。三段全部工作台可见 +
getJsonApiFormat 放行 + 首跑 SUCCESS：

| 段 | workflowId | 输入 |
|---|---|---|
| 1 reactor 换脸 | `2092594001879216130`（昨日） | 1.image / 2.image |
| 2 Klein 发型迁移 | `2092820988747919362`（新副本） | 597/598.image + **500.text 指令** |
| 3 scail2 表情复刻 | `2092820995869847553`（新副本） | 68.image + 2.video + 85/88.value |

源流 copy 零币；**810 解锁新知**：setContent 建版本+编辑器保存都不够，**必须
成功跑过一次**（klein 首跑 2092823688720703490 SUCCESS 后 gate 即开）；
编辑器加载检测用顶栏 "Save manually"/FPS 信号（canvas 计数已失效——新版
编辑器无 canvas 标签）。webapp 替代形态（ai-app/run 2075052610570244098 /
2072661793658462210）始终可用，效果同源。
