# -*- coding: utf-8 -*-
"""_h3_edrun.py — H3 工作流烤入运行(edrun4 模式适配 WF 2092847765977378817)。"""
import sys
import io
import json
import time
...
LOGF = open(HERE / "_h3_edrun.log", "a", encoding="utf-8")
...
with sync_playwright() as p:
    ...
    page.screenshot(path=str(HERE / "_h3_ed_ready.png"))
    if not ready:
    ...
    page.screenshot(path=str(HERE / "_h3_ed_dlg.png"))
    ...
    page.wait_for_timeout(10000)
    page.screenshot(path=str(HERE / "_h3_ed_after.png"))

    # H3 双采 10s ≈ 500-660s, 留 16 分钟
    ...
    page.screenshot(path=str(HERE / "_h3_ed_final.png"))
    ctx.close()
print("[outcome]", outcome, flush=True)
(HERE / "_h3_edrun_outcome.json").write_text(
