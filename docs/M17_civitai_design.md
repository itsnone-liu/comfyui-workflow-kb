...
（Workflows 资源 zip 匿名公开、与 RH 图同构可直接入库）和 M11 研究通道的
第四源（desc 正文是机制句富矿）；NSFW 域它近乎独占。不走 CLI，stdlib 直连。**
...
collector/batch_civitai.py    批量采集器：query 或 tag 驱动
                              → data/raw/civitai/<modelId>_<slug>/
                                 meta.json（资源元数据+全 versions+stats）
                                 desc.md（清洗后正文，技巧富矿）
...
- **normalizer 兼容性是 P2 唯一技术风险**：先跑 20 条样本看 unknown 节点分类
