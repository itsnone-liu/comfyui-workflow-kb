...
1. **LLM 用什么做知识卡分析？**
   - a) DeepSeek API（你有 key？）
   - b) 复用 OpenTutor 的 LLM 路由（多 provider）
   - c) 本地 Ollama（免费但质量降）
   - 建议 a，知识卡质量是库的命脉
2. **首批入库规模**：M1 先 50 条精选（热门+高使用数）还是直接抓 500？建议 50 起步控质量。
3. **本地有没有 ComfyUI 环境**？（决定 M5 何时可行；没有也可用 RunningHub 云端跑实验，但要花平台额度）
4. **语言**：知识卡中文为主（贴合你的检索习惯），节点名/参数名保留英文原文。默认按此执行。
